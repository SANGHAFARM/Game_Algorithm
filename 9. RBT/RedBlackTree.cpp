...
void RedBlackTree::Remove(int data)
{
	...
	{
		std::cout << "오류 : 노드 삭제 실패. 삭제할 노드를 찾지 못했습니다. (검색 값 : " 
			<< data << ")\n";
		return;
	}
...
void RedBlackTree::PrintRecursive(Node* node, int depth, int blackCount)
{
	...
	// 검은색 수 출력을 위한 문자열
	char blackCountStirng[50] = {};

	// 자손이 없으면 현재까지의 검은색 노드 수 설정
	if (node->Left() == nil && node->Right() == nil)
	{
		sprintf_s(blackCountStirng, "------ %d", blackCount);
	}
	...
	// 현재 노드 값 출력
	std::cout << node->Data() << " " << node->ColorString() 
		<< " [" << position << ", " << parentData << "] " << blackCountStirng << "\n";
